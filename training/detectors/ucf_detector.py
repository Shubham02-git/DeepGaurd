...
class UCFDetector(AbstractDetector):
    ...
    def get_train_metrics(self, data_dict: dict, pred_dict: dict) -> dict:
        ...
        return {'acc': acc, 'acc_spe': acc_spe, 'auc': auc, 'eer': eer, 'ap': ap}

    def _run_heads(self, f_spe, f_share):
        out_sha, sha_feat = self.head_sha(f_share)
        out_spe, spe_feat = self.head_spe(f_spe)
        return out_sha, sha_feat, out_spe, spe_feat
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        if inference:
            ...
            out_sha, sha_feat, out_spe, spe_feat = self._run_heads(f_spe, f_share)
            ...
            return {'cls': out_sha, 'feat': sha_feat}
        ...
        if aug_idx < 0.7:
            # real
            idx_list = list(range(bs//2))
            random.shuffle(idx_list)
            f_share[:bs//2] = f_share[idx_list]
            # fake
...
class AdaIN(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return (
            ((x - x_mean.expand(size)) / x_std.expand(size))
