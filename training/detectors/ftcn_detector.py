...
def valid_idx(idx, h):
    i = idx // h
    j = idx % h
    return not (j == 0 or i == h - 1 or j == h - 1)
...
class RandomAvgPool(nn.Module):
    ...
    def forward(self, x):
        ...
        k = 8 if self.training and random_select else len(candidates)
...
parameters = list(signature(nn.Conv3d).parameters)
print(parameters)
...
def temporal_only_conv(module, name, removed, stride_removed=0):
        ...
        if type(sub_module) == nn.Conv3d:
            ...
            if removed > spatial_count:
                ...
                if extra is not None:
                    bn_str = "bn" if attr_str == "conv" else f"{attr_str}_bn"
    ...
    for name, immediate_child_module in module.named_children():
        removed, stride_removed = temporal_only_conv(
            immediate_child_module, f"{old_name}.{name}", removed, stride_removed
        )
