...
# ⚠️ SECURITY: SCNet50 CDN URL disabled (unverified source)
# Manual download required for security:
#   1. Download from: https://github.com/JStehouwer/SCNet (official repo)
#   2. Or: Check STIL/SCNet GitHub releases for verified weights
#   3. Save to: ./pretrained/scnet50_v1d-4109d1e1.pth
#   4. Verify hash: certutil -hashfile pretrained/scnet50_v1d-4109d1e1.pth SHA256
model_urls = {
    # 'scnet50_v1d': 'https://backseason.oss-cn-beijing.aliyuncs.com/scnet/scnet50_v1d-4109d1e1.pth',  # DISABLED
}
...
def scnet50_v1d(num_segments, pretrained=False, **kwargs):
    ...
    model = SCNet(num_segments, SCBottleneck, [3, 4, 6, 3],
                   ...
                   avd=True, **kwargs)
    if pretrained:
        # ⚠️  SECURITY: Manual download required (CDN URL disabled)
