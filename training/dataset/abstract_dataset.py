...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def init_data_aug_method(self):
        trans = A.Compose([           
            ...
            A.Resize(height=self.config['resolution'], width=self.config['resolution'], p=0 if self.config['with_landmark'] else 1),
            A.OneOf([
        ...
        for label in dataset_info[dataset_name]:
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                def get_frame_index(path):
                    """Extract frame index from path, handle both numbered and hex filenames."""
                    filename = path.split('\\')[-1] if '\\' in path else path.split('/')[-1]
                    base_name = filename.split('.')[0]
                    try:
                        return int(base_name)
                    except ValueError:
                        # For non-numeric filenames, keep them in their original order
                        return hash(base_name) % 1000000
                
                frame_paths = sorted(frame_paths, key=get_frame_index)
